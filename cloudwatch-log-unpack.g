...
var headers
    data
        event = (asn city clientIP code correlationID country eventType host ip lat long method model path programIds profileID referer region resultCount searchTerm searchType seedId serverTime sessionAppName time totalResults userID user_agent_raw vendor)
        param = (path clipsAutoplayDisabled exclude feat feedTypes genres jwToken recToken kids maxRating nonKids offset profileId purchases q source started tripleNav tz)
...
#    os!exit
# normal
# 1768188872111,"Jan 12 03:34:31 ip-10-1-147-121 search[5528]: {""appName""
# sometimes
# 1610364810561,"Jan 11 11:33:30 ip-10-1-135-217 search: {""asn""
#
# comment out the normal split
    var tok (message(.split '\]: '))
    #var tok (message(.split 'search: '))
...
#            print (list @LINE jwtoken)
            cond
                (not (null? jwtoken))
                        ...
                        var decoded (os!exec 'bash' '-c' ("echo '%s' | jwtdecode.bin "(.format jwtoken)))
